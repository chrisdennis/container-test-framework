 ...
 */
package com.tc.test.server.appserver.jetty12x;
...
public class Jetty12xAppServer extends AbstractAppServer {
  ...
  public Jetty12xAppServer(final AppServerInstallation installation) {
    super(installation);
  }
  ...
  private AppServerResult startJetty(final AppServerParameters params) throws Exception {
    ...
    cmd.add("--module=http,ee10-deploy,logging-jetty,console-capture");
  ...
  private void prepareDeployment(final AppServerParameters params) throws Exception {
    ...
    File resourcesDir = new File(instanceDir, "resources");
    ensureDirectory(resourcesDir);
    writeLoggingProperties(resourcesDir);

    setProperties(params, jetty_port, instanceDir);
  }

  private void writeLoggingProperties(File resourcesDir) throws IOException {
    FileOutputStream fos = null;
    try {
      fos = new FileOutputStream(new File(resourcesDir, "jetty-logging.properties"));
      fos.write(new String("org.eclipse.jetty.LEVEL=INFO\n").getBytes());
    } finally {
  ...
  private static String contextFile(final String warFile, final String contextPath) {
    ...
    s += "<!DOCTYPE Configure PUBLIC \"-//Jetty//Configure//EN\" \"https://jetty.org/configure_10_0.dtd\">\n";
    s += "<Configure class=\"org.eclipse.jetty.ee10.webapp.WebAppContext\">\n";
